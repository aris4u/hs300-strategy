...
选股建议页的 K 线只画下面这些。看图用左列，不要再记一套「黄三角 / F / CT」当另一套图例。

| 图上 | 公式里 | 做什么 |
|---|---|---|
| 紫色圆点 **机会点** | `f_signal`（通达信叫 F） | 只提示有资金介入，**不买入** |
| 绿色三角 **建仓点** | `launch_turn`（通达信叫黄三角，图上画成绿色） | **唯一买入**。T 日收盘出信号，T+1 开盘买 |
| 红色倒三角 **减仓点** | `reduce_trend` 或 `reduce_band`（JC） | 减仓。趋势风险减到约 70%，波段风险减到约 50%。图上两种都是同一个红倒三角 |
| 蓝色菱形 **风险点** | `caution`（注意 YJ） | 只提示，**不自动减仓/清仓** |
| 深红色叉 **清仓点** | `escape_top`（CT）或 `take_profit`（止盈） | **清仓**。图上不区分是逃顶还是止盈 |

底色：浅绿 = 满仓；浅橙 = 已经减过仓。

评级 / MFE / MAE **不画在图上**，只做事后研究，**禁止**进实时筛选和当日动作。JC 减仓是风险控制，不是「趋势确认所以该加仓」。
